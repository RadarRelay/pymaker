...
@total_ordering
class Wad:
    """Represents a number with X decimal places, 18 by default.
    ...
    Notes:
        The internal representation of `Wad` is an unbounded integer, the last X digits of it being treated
        as decimal places. It is similar to the representation used in Maker contracts (`uint128`).
    """

    def __init__(self, value, decimals: int=18):
        ...
        if isinstance(value, Wad):
            ...
            self.value = int((Decimal(value.value) / (Decimal(10)**Decimal(27-decimals))).quantize(1, rounding=ROUND_DOWN))
    ...
    @classmethod
    def from_number(cls, number, decimals: int=18):
        assert((isinstance(decimals, int)) and (decimals >= 0))
        # assert(number >= 0)
    ...
    def __str__(self):
        tmp = str(self.value).zfill(self.decimals+1)
        return (tmp[0:(len(tmp)-self.decimals)] + "." + tmp[(len(tmp)-self.decimals):len(tmp)]).replace("-.", "-0.")

    def __add__(self, other):
        if isinstance(other, Wad) and self.decimals == 18 and other.decimals == 18:
    ...
    def __mul__(self, other):
        ...
        elif isinstance(other, Ray) and self.decimals == 18:
            result = Decimal(self.value) * Decimal(other.value) / (Decimal(10) ** Decimal(27))
            return Wad(int(result.quantize(1, rounding=ROUND_DOWN)))
        elif isinstance(other, int) and self.decimals == 18:
    ...
    def __int__(self):
        return int(self.value / 10**self.decimals)

    def __float__(self):
        return self.value / 10**self.decimals

    def __round__(self, ndigits: int = 0):
        return Wad(round(self.value, -self.decimals + ndigits))
